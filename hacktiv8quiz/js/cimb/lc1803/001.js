...
function highestGlobalMinimum(firstArr, secondArr) {
    var firstLowest = firstArr[0];
    var secondLowest = secondArr[0];
    for(var i = 0; i<firstArr.length;i++){
        if(firstArr[i] < firstLowest){
            firstLowest = firstArr[i];
        }
    }

    for(var i = 0; i<secondArr.length;i++){
        if(secondArr[i] < secondLowest){
            secondLowest = secondArr[i];
        }
    }
