...
function activityNotifications(expenditure, d) {
    var notification = 0;
    var maxPutaran = expenditure.length - d;
    for (var i = 0; i < maxPutaran; i++) { // i = 1,
        var newArr = [];
        for (var j = i; j < d + i; j++) {
            newArr.push(expenditure[j]);
        }
        ...
        if (newArr.length % 2 === 1) {
            ...
            median = (newArr[Math.floor(newArr.length / 2) - 1] + newArr[Math.floor(newArr.length / 2)]) / 2;
        }
        var today = expenditure[d + i];
        if (today >= median * 2) {
            notification++;
        }
        // console.log(newArr, median, today);
    }
    return notification
}
